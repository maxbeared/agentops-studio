...
export function StartNode({ data, selected }: NodeProps) {
  ...
  return (
    <div className={`relative rounded-lg border border-emerald-500 bg-emerald-500/10 p-3 min-w-[140px] ${selected ? 'ring-2 ring-emerald-400' : ''}`}>
...
export function LLMNode({ data, selected }: NodeProps) {
  const nodeData = data as WorkflowNodeData;
  return (
    <div className={`relative rounded-lg border border-blue-500 bg-blue-500/10 p-3 min-w-[140px] ${selected ? 'ring-2 ring-blue-400' : ''}`}>
      ...
      <p className="mt-1.5 text-xs text-zinc-400">{nodeData.config?.model || 'gpt-4'}</p>
    </div>
  );
}
...
export function RetrievalNode({ data, selected }: NodeProps) {
  const nodeData = data as WorkflowNodeData;
  return (
    <div className={`relative rounded-lg border border-purple-500 bg-purple-500/10 p-3 min-w-[140px] ${selected ? 'ring-2 ring-purple-400' : ''}`}>
      ...
      <p className="mt-1.5 text-xs text-zinc-400">Top-{nodeData.config?.topK || 5}</p>
    </div>
  );
}
...
export function ConditionNode({ data, selected }: NodeProps) {
  ...
  return (
    <div className={`relative rounded-lg border border-amber-500 bg-amber-500/10 p-3 min-w-[140px] ${selected ? 'ring-2 ring-amber-400' : ''}`}>
      ...
      {condition ? (
        <p className="mt-1.5 text-xs text-zinc-400 truncate" title={condition}>{condition}</p>
      ) : (
        <p className="mt-1.5 text-xs text-zinc-500 italic">No condition</p>
      )}
...
export function ReviewNode({ data, selected }: NodeProps) {
  const nodeData = data as WorkflowNodeData;
  return (
    <div className={`relative rounded-lg border border-pink-500 bg-pink-500/10 p-3 min-w-[140px] ${selected ? 'ring-2 ring-pink-400' : ''}`}>
      ...
      <p className="mt-1.5 text-xs text-zinc-400 truncate">{nodeData.config?.assigneeEmail || 'Unassigned'}</p>
    </div>
  );
}
...
export function WebhookNode({ data, selected }: NodeProps) {
  const nodeData = data as WorkflowNodeData;
  return (
    <div className={`relative rounded-lg border border-cyan-500 bg-cyan-500/10 p-3 min-w-[140px] ${selected ? 'ring-2 ring-cyan-400' : ''}`}>
      ...
      <p className="mt-1.5 text-xs text-zinc-400 truncate">{nodeData.config?.method || 'POST'}</p>
    </div>
  );
}
...
export function OutputNode({ data, selected }: NodeProps) {
  const nodeData = data as WorkflowNodeData;
  return (
    <div className={`relative rounded-lg border border-zinc-500 bg-zinc-500/10 p-3 min-w-[140px] ${selected ? 'ring-2 ring-zinc-400' : ''}`}>
...
export function InputNode({ data, selected }: NodeProps) {
  const nodeData = data as WorkflowNodeData;
  const fields = nodeData.config?.inputSchema?.fields || [];
  return (
    <div className={`relative rounded-lg border border-teal-500 bg-teal-500/10 p-3 min-w-[140px] ${selected ? 'ring-2 ring-teal-400' : ''}`}>
      ...
      <p className="mt-1.5 text-xs text-zinc-400">{fields.length} field{fields.length !== 1 ? 's' : ''}</p>
    </div>
  );
}
...
export function TextNode({ data, selected }: NodeProps) {
  const nodeData = data as WorkflowNodeData;
  const operation = nodeData.config?.operation || 'trim';
  return (
    <div className={`relative rounded-lg border border-orange-500 bg-orange-500/10 p-3 min-w-[140px] ${selected ? 'ring-2 ring-orange-400' : ''}`}>
      ...
      <p className="mt-1.5 text-xs text-zinc-400 capitalize">{operation}</p>
    </div>
  );
}
...
export function LoopNode({ data, selected }: NodeProps) {
  const nodeData = data as WorkflowNodeData;
  const maxIterations = nodeData.config?.maxIterations || 100;
  return (
    <div className={`relative rounded-lg border border-green-500 bg-green-500/10 p-3 min-w-[140px] ${selected ? 'ring-2 ring-green-400' : ''}`}>
      ...
      <p className="mt-1.5 text-xs text-zinc-400">Max: {maxIterations}</p>
    </div>
  );
}
...
export function DelayNode({ data, selected }: NodeProps) {
  ...
  return (
    <div className={`relative rounded-lg border border-yellow-500 bg-yellow-500/10 p-3 min-w-[140px] ${selected ? 'ring-2 ring-yellow-400' : ''}`}>
      ...
      <p className="mt-1.5 text-xs text-zinc-400">{duration} {unit}</p>
    </div>
  );
}
...
export function TransformNode({ data, selected }: NodeProps) {
  const nodeData = data as WorkflowNodeData;
  const template = nodeData.config?.template || '';
  return (
    <div className={`relative rounded-lg border border-indigo-500 bg-indigo-500/10 p-3 min-w-[140px] ${selected ? 'ring-2 ring-indigo-400' : ''}`}>
      ...
      {template ? (
        <p className="mt-1.5 text-xs text-zinc-400 truncate" title={template}>{template.slice(0, 20)}...</p>
      ) : (
        <p className="mt-1.5 text-xs text-zinc-500 italic">No template</p>
      )}
    </div>
  );
}
...
export function CodeNode({ data, selected }: NodeProps) {
  ...
  return (
    <div className={`relative rounded-lg border border-red-500 bg-red-500/10 p-3 min-w-[140px] ${selected ? 'ring-2 ring-red-400' : ''}`}>
      ...
      <p className="mt-1.5 text-xs text-zinc-400">{language}</p>
    </div>
  );
}
...
export function MergeNode({ data, selected }: NodeProps) {
  const nodeData = data as WorkflowNodeData;
  const strategy = nodeData.config?.strategy || 'all';
  return (
    <div className={`relative rounded-lg border border-violet-500 bg-violet-500/10 p-3 min-w-[140px] ${selected ? 'ring-2 ring-violet-400' : ''}`}>
      ...
      <p className="mt-1.5 text-xs text-zinc-400 capitalize">{strategy}</p>
    </div>
  );
}
...
export function ErrorHandlerNode({ data, selected }: NodeProps) {
  const nodeData = data as WorkflowNodeData;
  return (
    <div className={`relative rounded-lg border border-rose-500 bg-rose-500/10 p-3 min-w-[140px] ${selected ? 'ring-2 ring-rose-400' : ''}`}>
